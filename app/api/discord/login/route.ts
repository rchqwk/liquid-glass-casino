...
function requireEnv(name: string): string {
  ...
  if (!v) {
    console.error(`[Discord Login] Missing environment variable: ${name}`);
    throw new Error(`Server configuration error: ${name} is not set. Please add your Discord application credentials to .env.local`);
  }
  return v;
}
...
export async function POST(req: Request) {
  try {
    ...
    const message = e?.message ?? "Discord login failed";
