...
export function BlackjackTablePageClient({
  ...
  const showHorizontalStakeDock = horizontalMode && !!state && state.phase === "betting" && !!mySeat;
                  ...
                  {myHasLockedStake ? (
                    <div className="mt-3 flex flex-wrap items-end gap-2">
                      <div className="rounded-2xl border border-amber-400/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                        <div className="text-[10px] uppercase tracking-wide opacity-70">Locked stake</div>
                        <div className="mt-1 font-mono text-sm font-semibold">{Number(mySeat?.bet ?? 0).toFixed(2)} ⓒ</div>
                      </div>
                      <button
                        type="button"
                        className="glass-soft rounded-2xl border border-rose-400/20 bg-rose-500/10 px-4 py-2 text-sm font-semibold text-rose-100 hover:bg-rose-500/15 disabled:opacity-40"
                        disabled={betPending}
                        onClick={clearBetWithWallet}
                      >
                        Clear stake
                      ...
                      <button
                        type="button"
                        className="glass-soft rounded-2xl px-4 py-2 text-sm font-semibold text-white/80 hover:bg-white/10"
                        onClick={() => setAllIn(false)}
                        title="Clear the all-in toggle for the next bet"
                      >
                        Not all-in
                      </button>
                    </div>
